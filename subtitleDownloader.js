...
async function downloadSubtitleWithYtdlCore(videoId, language) {
    try {
        ...
        if (manualTrack) {
            ...
            subtitleUrl = `${manualTrack.baseUrl}&fmt=json3`;
            ...
            if (autoTrack) {
                ...
                subtitleUrl = `https://www.youtube.com/api/timedtext?lang=${autoTrack.languageCode}&v=${videoId}&fmt=json3&tlang=${language}&kind=asr&name=${encodeURIComponent(autoTrack.name?.simpleText || '')}`;
                ...
                if (firstManualTrack) {
                    ...
                    subtitleUrl = `${firstManualTrack.baseUrl}&fmt=json3&tlang=${language}`;
            ...
            try {
                ...
                if (!response.data || !response.data.events) {
                    ...
                    if (isAutoGenerated) {
                        ...
                        if (manualTrack) {
                            subtitleUrl = `${manualTrack.baseUrl}&fmt=json3&tlang=${language}`;
                ...
                // Convert JSON3 format to VTT
                const vttContent = convertJson3ToVtt(response.data);
                if (!vttContent) {
                    throw new Error('Không thể chuyển đổi JSON3 sang VTT');
                }
...
// Helper function to convert YouTube's JSON3 format to VTT
    ...
    try {
        ...
        for (const event of jsonData.events) {
            if (event.segs && Array.isArray(event.segs)) {
                const startTime = msToTime(event.tStartMs);
                const endTime = msToTime(event.tStartMs + event.dDurationMs);
        ...
        logger.error(`Error converting JSON3 to VTT: ${error.message}`);
        return null;
    }
}

// Helper function to convert milliseconds to VTT time format
        ...
        if (platform === 'youtube') {
            ...
            (async () => {
                try {
                    ...
                    success = await downloadSubtitleWithYtDlp(videoId, language);
        ...
        for (const lang of allLanguages) {
            try {
                ...
                if (!subtitleContent) {
                    ...
                    subtitleContent = await downloadSubtitleWithYtDlp(videoId, lang);
                }
